...
import { Key, Save, AlertCircle, Eye, EyeOff, Settings, RefreshCw } from 'lucide-react';
...
const APIManager = () => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success('Configuração salva com sucesso! O chatbot será atualizado automaticamente.');
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">API Manager</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Gerencie suas chaves de API e configurações de IA
          </p>
        </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {/* Custom Prompt */}
        <Card title="Prompt Personalizado" icon={Settings}>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Instruções Especiais para o Chatbot
            </label>
            <textarea
              value={config.custom_prompt}
              onChange={(e) => handleChange('custom_prompt', e.target.value)}
              placeholder="Ex: Sempre coletar nome completo do cliente. Perguntar sobre interesse em agendamento. Usar tom profissional mas amigável..."
              rows={6}
              className="w-full px-4 py-2 border rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 font-mono text-sm"
            />
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Instruções adicionais que serão incluídas no prompt do sistema do chatbot.
            </p>
          </div>
        </Card>
